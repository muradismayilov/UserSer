package com.muradismayilov.userservice.controller;

import java.util.List;


import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestController;

import com.muradismayilov.userservice.business.abstracts.UserTypesService;

import com.muradismayilov.userservice.business.requests.CreateUserTypesRequest;
import com.muradismayilov.userservice.business.responses.GetAllUserTypesResponse;


import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.NoArgsConstructor;

@Data
@AllArgsConstructor
@NoArgsConstructor
@RestController
@RequestMapping("/users/usertypes")
public class UserTypesController {
	
	private UserTypesService usertypesservice;

	@GetMapping
	public List<GetAllUserTypesResponse> getAll(){
		return usertypesservice.getAll();
	}
	
	@PostMapping()
	@ResponseStatus(code = HttpStatus.CREATED)
	public void add(@RequestBody CreateUserTypesRequest createusertypesrequest) {
		this.usertypesservice.add(createusertypesrequest);
	}
}
